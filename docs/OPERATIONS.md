...
There's one stack, `docker-compose.yml`, used both locally and in
production — no separate dev-only Postgres/seed setup.

```bash
cp .env.example .env         # set AUTH_SECRET (openssl rand -base64 32) and the rest
docker compose up -d --build # http://localhost (nginx proxies admin + backend)
```

Create the first OWNER account by visiting `/signup` once — see the README's
Deployment section. OTP codes and all SMS print to the backend container's
logs (`SMS_PROVIDER=console`).

To exercise the scheduled jobs manually:

```bash
curl -X POST -H "Authorization: Bearer <CRON_SECRET>" http://localhost/api/cron
```
...
npm run test:e2e  # Playwright booking flow (first run: npx playwright install chromium)
```
...
4. First boot: open the admin site's `/signup` page once and create the OWNER
   account — the first web registration becomes the owner and creates the
   studio row. (`/signup` closes itself afterwards.)
5. Load the class timetable:
   `docker compose exec app sh -c "cd /app && node_modules/.bin/tsx prisma/import-schedule.ts"`
   — creates the studio locations, trainers, class types and weekly recurrence
   rules, then generates sessions from today. Safe to re-run; edit the schedule
   in `backend/src/lib/schedule.ts` and re-run.
6. **Paystack dashboard** → Settings → API Keys & Webhooks → set the webhook
   URL to `https://<your-domain>/api/webhooks/paystack`. Use test keys until
   go-live.
7. SMS sender ID must be pre-registered with Arkesel/Hubtel or messages
   will be rejected by the carrier (SMS is dormant until a provider is configured).
